...
function ShopContent() {
  ...
  return (
    <main className="flex-1 max-w-[1360px] mx-auto px-3 sm:px-6 w-full py-4 sm:py-6">
      {/* Breadcrumb & Header */}
      <div className="mb-4">
        <div className="text-xs text-slate-500 dark:text-slate-500 mb-1.5 font-medium">
          <span>{t('nav.home')}</span> &rarr; <span className="text-emerald-800 dark:text-emerald-700 font-extrabold">{t('nav.shop')}</span>
        </div>
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            >
              <SlidersHorizontal className="w-4 h-4 text-emerald-800 dark:text-emerald-700" />
              <span>Filter</span>
            </button>
            ...
            <div className="hidden sm:flex border border-slate-200 dark:border-slate-800/80 rounded-xl bg-white dark:bg-slate-950 p-1">
              <button
                ...
                className={`p-1.5 rounded-lg text-slate-600 dark:text-slate-600 ${viewMode === 'grid' ? 'bg-emerald-800 dark:bg-emerald-700 text-white' : 'hover:bg-slate-100 dark:hover:bg-slate-900/60'}`}
              >
                <Grid className="w-4 h-4" />
              </button>
              <button
                onClick={() => setViewMode('list')}
                className={`p-1.5 rounded-lg text-slate-600 dark:text-slate-600 ${viewMode === 'list' ? 'bg-emerald-800 dark:bg-emerald-700 text-white' : 'hover:bg-slate-100 dark:hover:bg-slate-900/60'}`}
              >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 lg:gap-6">
        ...
        <aside className={`lg:col-span-3 bg-white dark:bg-slate-950 p-5 rounded-2xl border border-slate-200/80 dark:border-slate-800/80 shadow-xs space-y-5 ${
          ...
          <div>
            ...
            <div className="space-y-1 text-xs text-slate-600 dark:text-slate-600">
              <button
                onClick={() => { updateParam('category', ''); setIsMobileFilterOpen(false); }}
                className={`w-full text-left py-1.5 px-2.5 rounded-lg transition ${!selectedCategory ? 'bg-emerald-100/70 dark:bg-emerald-950/80 text-emerald-900 dark:text-emerald-700 font-extrabold' : 'hover:bg-slate-50 dark:hover:bg-slate-900/60 font-medium'}`}
              >
                Alle Kategorien
              </button>
              {categories.map((cat) => (
                <button
                  ...
                  className={`w-full text-left py-1.5 px-2.5 rounded-lg transition ${selectedCategory === cat.slug ? 'bg-emerald-100/70 dark:bg-emerald-950/80 text-emerald-900 dark:text-emerald-700 font-extrabold' : 'hover:bg-slate-50 dark:hover:bg-slate-900/60 font-medium'}`}
                >
                  {cat.name}
                </button>
              ))}
          ...
          <div className="border-t border-slate-100 dark:border-slate-800 pt-3.5">
            ...
            <div className="space-y-1 text-xs text-slate-600 dark:text-slate-600">
              <button
                ...
                className={`w-full text-left py-1.5 px-2.5 rounded-lg transition ${!selectedBrand ? 'bg-emerald-100/70 dark:bg-emerald-950/80 text-emerald-900 dark:text-emerald-700 font-extrabold' : 'hover:bg-slate-50 dark:hover:bg-slate-800 font-medium'}`}
              >
                Alle Marken
              </button>
              {brands.map((b) => (
                <button
                  ...
                  className={`w-full text-left py-1.5 px-2.5 rounded-lg transition ${selectedBrand.toLowerCase() === b.name.toLowerCase() ? 'bg-emerald-100/70 dark:bg-emerald-950/80 text-emerald-900 dark:text-emerald-700 font-extrabold' : 'hover:bg-slate-50 dark:hover:bg-slate-800 font-medium'}`}
                >
                  {b.name}
                </button>
              ))}
          ...
          <div className="border-t border-slate-100 dark:border-slate-800 pt-3.5">
            <div className="flex justify-between items-center mb-2 text-xs">
              ...
              <span className="font-extrabold text-emerald-800 dark:text-emerald-700">{priceRange} €</span>
            </div>
            <input
              type="range"
              min="100"
        ...
        <div className="lg:col-span-9">
          {isLoading ? (
            <div className="py-16 text-center text-slate-500 dark:text-slate-500 text-xs font-semibold">
