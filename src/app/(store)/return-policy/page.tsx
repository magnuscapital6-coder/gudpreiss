...
export default function ReturnPolicyPage() {
  ...
  useEffect(() => {
    async function loadData() {
      setIsLoading(true);
      try {
        const data = await getLegalPage('return-policy');
        setPage(data);
      } catch (err) {
        console.error(err);
      } finally {
      ...
      <main className="flex-1 max-w-4xl mx-auto px-4 sm:px-6 w-full py-12 space-y-8">
        <div className="text-center space-y-3">
          <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-800 dark:text-emerald-400 text-xs font-extrabold">
            ...
            <span>14 TAGE WIDERRUFSRECHT &amp; RÜCKGABE</span>
          </div>
          <h1 className="text-3xl sm:text-4xl font-black tracking-tight">{page?.title || 'Widerrufsbelehrung'}</h1>
          <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 max-w-lg mx-auto">
            {page?.subtitle || 'Informationen zum gesetzlichen Widerrufsrecht für Verbraucher.'}
          </p>
        </div>
